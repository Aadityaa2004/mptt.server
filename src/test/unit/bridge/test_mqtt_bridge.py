...
def _add_bridge_to_sys_path() -> None:
    ...
    within the src/test subtree.
    """
    here = os.path.dirname(__file__)
    # Walk up to repo root: src/test/unit/bridge -> src/test/unit -> src/test -> src -> repo root
    repo_root = os.path.abspath(os.path.join(here, "..", "..", "..", ".."))
    bridge_dir = os.path.join(repo_root, "src", "production", "MQT.Bridge")
    if os.path.isdir(bridge_dir):
        sys.path.insert(0, bridge_dir)

        # Add vendor for paho-mqtt when installed via pip install --target vendor
